-- 01. отображает все столбцы в таблице aircrafts
SELECT * FROM aircrafts;

-- Отображает только выбранные столбцы в таблице aircrafts
SELECT aircraft_code, model
FROM aircrafts;

-- 02. Получение конкретных строк в таблице
SELECT model, range
...
-- 03. Получение диапазона значений
...
AND scheduled_departure 
    between '2017-08-31' and '2017-09-01';

-- 05. Работа со значениями NULL
'''
-- SELECT
...
SELECT
    scheduled_departure, flight_no, departure_airport, arrival_airport
FROM bookings. flights
WHERE departure_airport = 'DME'
...
-- 07. Устранение дублирования строк
SELECT DISTINCT
    departure_airport, arrival_airport
FROM bookings.flights
ORDER BY 1,2;
...
-- 09. Устранение дублирования строк
SELECT DISTINCT
    departure_airport, arrival_airport
FROM bookings.flights
ORDER BY 1,2;
...
SELECT
    AVG (amount) AS Average, SUM (amount) as Summary
...
SELECT
    departure_airport, count (actual_arrival)
FROM bookings.flights
...
SELECT
    departure_airport, arrival_airport, count (actual_arrival)
FROM bookings.flights
